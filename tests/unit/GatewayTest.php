...
class GatewayTest extends \Codeception\Test\Unit
{
    ...
    public function testValidateRepositoryUrl() {
        $message = 'Url must have scheme, host and path. It cannot contain a query string nor a hash fragment';

        $url = 'https://wendig.io/my/doc.xml?version=2';
        try {
            \SRG\Gateway::initiate($url);
            $this->fail("Expected exception \SRG\Exception not thrown");
